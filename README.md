...
* 使用antlr4编译生成SqlBaseParser代码
    ...
    ```shell
    maven -> spark project calalyst -> plugins -> antlr4 -> antlr4:antlr4
    ```
...
22/05/08 17:20:24 WARN [main] PlanChangeLogger:
...
执行sql
```sql
select distinct a11, a2+1 as a21, 'abc' as a33 from (select a1+1 as a11, a2 from zhu_guan_bing0 where a1 > 1 and 1=1) where a11 > 1 except select b1, b2, 'cba' as b3 from zhu_guan_bing1;
```

部分执行结果
```shell
22/05/08 17:42:39 WARN [main] PlanChangeLogger:
...
执行命令
```shell
spark-sql --jars /home/student5/zhu_guan_bing/spark-sql-job/SparkJob-1.0-SNAPSHOT.jar --conf spark.sql.extensions=org.bigdata.sparkjob.MySparkSessionExtension
```

```sql
select a1*1 from zhu_guan_bing0;
```

部分执行结果
```shell
22/05/08 18:44:26 WARN [main] MySparkRule: running MySparkRule
22/05/08 18:44:26 WARN [main] PlanChangeLogger:
